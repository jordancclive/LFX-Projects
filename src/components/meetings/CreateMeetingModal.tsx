...
import { X as CloseIcon, CheckCircle as CheckCircleIcon, Users as UsersIcon, FileText as FileTextIcon, Calendar as CalendarIcon, Settings as SettingsIcon, Check as CheckIcon, Search as SearchIcon, ChevronDown as ChevronDownIcon, FileIcon, LinkIcon } from 'lucide-react';
export const CreateMeetingModal = ({
  ...
  const [showLinksDocsOptions, setShowLinksDocsOptions] = useState(false);
  const [linksDocsSearchTerm, setLinksDocsSearchTerm] = useState('');
  const committeeInputRef = useRef(null);
  const individualInputRef = useRef(null);
  const attachmentInputRef = useRef(null);
  const linksDocsInputRef = useRef(null);
  const committeeDropdownRef = useRef(null);
  const individualDropdownRef = useRef(null);
  const attachmentDropdownRef = useRef(null);
  const linksDocsDropdownRef = useRef(null);
  ...
  // Sample data for Links & Docs
  const allLinksDocsItems = [{
    id: 1,
    ...
    id: 5,
    title: 'Project Roadmap 2023-2024',
    type: 'doc',
    category: 'Planning',
  ...
  const sortedLinksDocsItems = [...allLinksDocsItems].sort((a, b) => a.title.localeCompare(b.title));
  ...
  // Filtered Links & Docs items based on search term
  const filteredLinksDocsItems = sortedLinksDocsItems.filter(item => item.title.toLowerCase().includes(linksDocsSearchTerm.toLowerCase()) || item.category.toLowerCase().includes(linksDocsSearchTerm.toLowerCase()) || item.author.toLowerCase().includes(linksDocsSearchTerm.toLowerCase()));
  ...
  const handleLinksDocsToggle = item => {
    setFormData(prev => {
      const isSelected = prev.selectedLinksDocsAttachments.some(selectedItem => selectedItem.id === item.id);
      if (isSelected) {
        return {
          ...prev,
          selectedLinksDocsAttachments: prev.selectedLinksDocsAttachments.filter(selectedItem => selectedItem.id !== item.id)
        };
      } else {
        return {
          ...prev,
          selectedLinksDocsAttachments: [...prev.selectedLinksDocsAttachments, item]
        };
      }
    });
    // Clear search term when an option is selected
    setLinksDocsSearchTerm('');
  };
  ...
  const handleRemoveLinksDocsItem = (itemId, e) => {
    e.stopPropagation();
    setFormData(prev => ({
      ...prev,
      selectedLinksDocsAttachments: prev.selectedLinksDocsAttachments.filter(item => item.id !== itemId)
    }));
  };
  ...
  const steps = [{
    ...
    fields: <div className="space-y-6">
          ...
          {/* Links & Docs Search Section */}
          <div>
            <label className="block text-sm font-medium text-slate-700 mb-1">
              Add from Links & Docs
            </label>
            <div className="relative" ref={linksDocsInputRef}>
              <div className="min-h-[40px] flex flex-wrap items-center gap-2 w-full px-3 py-2 border border-slate-300 rounded-md focus-within:ring-2 focus-within:ring-azure-500 focus-within:border-azure-500 cursor-text" onClick={() => {
            setShowLinksDocsOptions(true);
            if (linksDocsInputRef.current.querySelector('input')) {
              linksDocsInputRef.current.querySelector('input').focus();
            }
          }}>
                {formData.selectedLinksDocsAttachments.length > 0 && formData.selectedLinksDocsAttachments.map(item => <div key={item.id} className="flex items-center bg-slate-100 px-2 py-1 rounded-md">
                      <span className="text-sm text-slate-700">
                        {item.title}
                      </span>
                      <button type="button" className="ml-1 text-slate-500 hover:text-slate-700" onClick={e => handleRemoveLinksDocsItem(item.id, e)}>
                        <CloseIcon className="h-3 w-3" />
                      </button>
                    </div>)}
                <div className="flex-1 min-w-[120px]">
                  <div className="flex items-center">
                    {(formData.selectedLinksDocsAttachments.length === 0 || linksDocsSearchTerm) && <SearchIcon className="h-4 w-4 text-slate-400 mr-2" />}
                    <input type="text" value={linksDocsSearchTerm} onChange={e => setLinksDocsSearchTerm(e.target.value)} onFocus={() => setShowLinksDocsOptions(true)} className="flex-1 outline-none text-sm w-full" placeholder={formData.selectedLinksDocsAttachments.length > 0 ? '' : 'Search Links & Docs'} />
                  </div>
                </div>
              </div>
              {showLinksDocsOptions && <div ref={linksDocsDropdownRef} className="fixed z-50 mt-1 w-[calc(100%-12px)] bg-white border border-slate-200 rounded-md shadow-lg max-h-60 overflow-y-auto" style={{
            width: linksDocsInputRef.current ? linksDocsInputRef.current.offsetWidth + 'px' : 'auto',
            left: linksDocsInputRef.current ? linksDocsInputRef.current.getBoundingClientRect().left + 'px' : '0',
            top: linksDocsInputRef.current ? linksDocsInputRef.current.getBoundingClientRect().bottom + 8 + 'px' : '0'
          }}>
                  {filteredLinksDocsItems.length > 0 ? filteredLinksDocsItems.map(item => <div key={item.id} className="flex items-center px-3 py-2 hover:bg-slate-50 cursor-pointer" onClick={() => handleLinksDocsToggle(item)}>
                        <div className="h-4 w-4 border border-slate-300 rounded mr-2 flex items-center justify-center">
                          {formData.selectedLinksDocsAttachments.some(selectedItem => selectedItem.id === item.id) && <CheckIcon className="h-3 w-3 text-azure-500" />}
                        </div>
                        <div className="flex items-center">
                          {item.type === 'doc' ? <FileIcon className="h-4 w-4 text-slate-400 mr-2" /> : <LinkIcon className="h-4 w-4 text-slate-400 mr-2" />}
                          <div>
                            <span className="text-sm text-slate-700">
                              {item.title}
                            </span>
  ...
  useEffect(() => {
    const handleClickOutside = e => {
      ...
      if (linksDocsDropdownRef.current && !linksDocsDropdownRef.current.contains(e.target) && linksDocsInputRef.current && !linksDocsInputRef.current.contains(e.target)) {
        setShowLinksDocsOptions(false);
      }
    };
